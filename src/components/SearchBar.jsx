...
const typeIcons = { tool: Wrench, tutorial: BookOpen, prompt: FileText }
const typeColors = { tool: 'text-primary', tutorial: 'text-green-600', prompt: 'text-purple-600' }

export default function SearchBar({ onClose, large = false }) {
    ...
    <div className="relative w-full max-w-2xl mx-auto" ref={ref}>
      ...
      {results.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl border border-gray-100 shadow-xl z-50 overflow-hidden">
          ...
          <Link
            to={`/ai-tools?q=${encodeURIComponent(q)}`}
            onClick={() => { onClose?.() }}
